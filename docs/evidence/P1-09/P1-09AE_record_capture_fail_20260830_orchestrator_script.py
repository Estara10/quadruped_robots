#!/usr/bin/env python3
"""P1-09AE — single real Runtime Record Capture orchestrator.

One simulation-only run:
  MuJoCo (scene_flat.xml, interface lo) + ROS2 simulation controller
  -> existing HUD + existing two-phase runtime recorder
  -> STOP sampling -> normal process exit -> real process facts -> FINALIZE
  -> post_run_summary on the saved record only.

Mechanism note (recorded for evidence): the task requires the recorder STOP
(sampling) before the controller exits, and FINALIZE only after the real process
facts are known.  The existing thin CLI ``record_runtime_run.py`` couples
``stop_sampling()`` + ``finalize()`` in one signal path, so it cannot separate
those two phases.  This orchestrator therefore drives the SAME existing
recorder lifecycle through its module API (``run_record.RunRecordRecorder`` +
``abs_rt_frame.read_shm_frame``).  No existing code/config/schema/controller is
modified; no retry is performed; a failed run is recorded faithfully.

No benchmark, pilot, formal experiment, FormalRunWriter, or real robot.
"""
...
def wait_rc(p: subprocess.Popen, timeout: float, name: str):
    """Wait up to timeout s; log the actual wait return code (None on timeout)."""
    try:
...
def pgid_of(pid: int):
    """Best-effort process group id, captured at launch for later killpg."""
    try:
...
def sigdisp(pid: int) -> None:
    """Capture the process signal disposition (SigIgn/SigCgt) before signals."""
    try:
...
def main() -> int:
    ...
    # No existing MuJoCo/ROS launch processes must be running.
    ...
    mujoco_env = {
        **os.environ,
        "LD_LIBRARY_PATH": f"{SDK_LIB}:{TORCH_LIB}:" + os.environ.get("LD_LIBRARY_PATH", ""),
    }
    ...
    mujoco = subprocess.Popen(
        ...
        cwd=str(MUJOCO_DIR), env=mujoco_env, start_new_session=True,
    ...
    def stop_group(p, name: str, saved_pid, saved_pgid, first_sig: int, timeout_s: float) -> tuple:
        """Normal-exit path first: SIGINT to the group leader pid. On timeout,
        escalate to SIGTERM for the whole process group (cleanup only)."""
        sigdisp(p.pid)
    ...
    src = "recorder_stop;SIGINT_ros_launch_group;SIGINT_mujoco_group"
    if ros_forced:
