#!/usr/bin/env python3
"""P1-09 closure — one real capture wired to the P1-02 FormalRunWriter/validator.

Chain: real MuJoCo + StateRL → /mujoco_rt_frame → two-phase JSONL runtime
record → authoritative structured events → P1-02 FormalRunWriter → P1-02
validator → formal verdict.

Only this run's launch environment is changed (child LD_LIBRARY_PATH, matching
scripts/launch_abs_sim.sh). No project code/config/schema/controller is
modified. One run only; no retry. No benchmark/pilot/formal-experiment/real
robot. The P1-02 validator verdict (expected INVALID when authoritative
collision/fall/goal/timeout/simulation_time sources are absent) is recorded
faithfully.
"""
...
CAP = Path("/tmp/p1_09_formal_capture")
...
CONTEXT_PATH = CAP / "manifest_context.json"
FORMAL_RUN_DIR = CAP / "formal_run"
VERDICT_PATH = CAP / "formal_verdict.json"
...
def wait_rc(p, timeout: float, name: str):
    try:
...
def main() -> int:
    log("P1-09C_BEGIN scene=scene_flat.xml interface=lo launch=mujoco.launch.py simulation_test=0")
    ...
    out = subprocess.run(["pgrep", "-af", PGREP_PAT], capture_output=True, text=True).stdout.strip()
    ...
    ros_cmd = bash_src() + f"cd {ROS2_WS}; exec ros2 launch rl_quadruped_controller mujoco.launch.py simulation_test:=0"
    ros = subprocess.Popen(["bash", "-lc", ros_cmd], start_new_session=True,
    ...
    hud = subprocess.Popen(["bash", "-lc", f"cd {SCRIPTS}; exec python3 abs_live_hud.py --iters 60"],
                           start_new_session=True, stdout=open(HUD_LOG, "wb"), stderr=subprocess.STDOUT)
    ...
    pub_cmd(2); time.sleep(0.5); pub_cmd(2)
    log("WAIT_FIXEDDOWN_TO_FIXEDSTAND"); time.sleep(4)
    pub_cmd(3, ly=1.0); time.sleep(0.5); pub_cmd(3, ly=1.0)
    log("WAIT_FIXEDSTAND_TO_RL"); time.sleep(3)
    ...
    # ------------------------------------------------------------- 9. normal exit
    def stop_group(p, name: str, saved_pid, saved_pgid, first_sig: int, timeout_s: float):
        sigdisp(p.pid)
    ...
    shutdown_monotonic_ns = time.monotonic_ns()
    log(f"SHUTDOWN_MONOTONIC_NS {shutdown_monotonic_ns}")
    ...
    # ------------------------------------------------------------- 10. facts (incl. shutdown_monotonic_ns)
    ...
    facts = {
        ...
        "shutdown_monotonic_ns": shutdown_monotonic_ns,
    }
    ...
    # ------------------------------------------------------------- 13. manifest context + formal binding
    ctx = subprocess.run(["python3", "build_runtime_manifest_context.py", "--output", str(CONTEXT_PATH)],
                         cwd=str(SCRIPTS), capture_output=True, text=True, timeout=60)
    log(f"CONTEXT_BUILD rc={ctx.returncode} stderr=[{ctx.stderr.strip()[:200]}]")
    if ctx.returncode == 0:
        bind = subprocess.run(
            ["python3", "formal_runtime_binding.py", str(RECORD_PATH),
             "--facts", str(FACTS_PATH), "--context", str(CONTEXT_PATH),
             "--run-dir", str(FORMAL_RUN_DIR)],
            cwd=str(SCRIPTS), capture_output=True, text=True, timeout=120)
        VERDICT_PATH.write_text(bind.stdout, encoding="utf-8")
        log(f"FORMAL_BINDING rc={bind.returncode} verdict=[{bind.stdout.strip()[:400]}]")
    else:
        log("FORMAL_BINDING SKIPPED (context build failed)")

    log(f"STATUS ros_rc={ros_rc} mujoco_rc={mujoco_rc} forced={forced} shutdown_complete={shutdown_complete} "
