//
// ABS Recovery Policy — 49-dim observation, no timer, no ray2d
//
...
template <typename T>
std::vector<T> ReadVectorFromYamlRec(const YAML::Node& node)
{
    std::vector<T> values;
...
StateRLRec::StateRLRec(CtrlInterfaces& ctrl_interfaces,
                       CtrlComponent& ctrl_component,
                       const std::vector<double>& target_pos) :
    FSMState(FSMStateName::RL_REC, "rl_rec", ctrl_interfaces),
    ...
    estimator_(ctrl_component.estimator_)
{
    ...
    RCLCPP_INFO(node_->get_logger(), "StateRLRec: Loading recovery model: %s", params_.model_name.c_str());
    model_ = torch::jit::load(model_path + "/" + params_.model_name);

    RCLCPP_INFO(node_->get_logger(), "[REC-VERIFY] num_observations=%d, num_of_dofs=%d",
                params_.num_observations, params_.num_of_dofs);
    RCLCPP_INFO(node_->get_logger(), "[REC-VERIFY] observations: %s",
                [&]() { std::string s; for (auto& o : params_.observations) s += o + " "; return s; }().c_str());
    RCLCPP_INFO(node_->get_logger(), "[REC-VERIFY] rl_kp[0]=%.2f, rl_kd[0]=%.2f",
...
void StateRLRec::run(const rclcpp::Time&, const rclcpp::Duration&)
{
    getState();
    if (!use_rl_thread_)
    {
        // Decimation: runModel() must be called at frequency/decimation Hz
        // to match training frequency
...
FSMStateName StateRLRec::checkChange()
{
    ...
    if (rl_step_count_ > ctrl_component_.recovery_steps) {
        RCLCPP_INFO(rclcpp::get_logger("StateRLRec"),
            "[REC-AUTO] returning to RL after %d steps", rl_step_count_);
        return FSMStateName::RL;
    }
    switch (ctrl_interfaces_.control_inputs_.command)
    {
    case 1:  return FSMStateName::PASSIVE;
    case 2:  return FSMStateName::FIXEDDOWN;
    case 3:  return FSMStateName::RL;
    default: return FSMStateName::RL_REC;
    }
}
...
torch::Tensor StateRLRec::computeObservation()
{
    ...
    if (!obs_dim_printed) {
        RCLCPP_INFO(rclcpp::get_logger("StateRLRec"), "[REC-OBS] computed shape: %ldx%ld (expected 1x%d)",
...
void StateRLRec::loadYaml(const std::string& config_path)
{
    ...
    {
        RCLCPP_ERROR(rclcpp::get_logger("StateRLRec"), "Config not found: %s", config_path.c_str());
        return;
    }
    ...
    params_.observations = ReadVectorFromYamlRec<std::string>(config["observations"]);
    params_.clip_obs = config["clip_obs"].as<double>();

    if (config["observations_history"].IsNull())
        params_.observations_history = {};
    else
        params_.observations_history = ReadVectorFromYamlRec<int>(config["observations_history"]);
    ...
    else
    {
        params_.clip_actions_upper = torch::tensor(
            ReadVectorFromYamlRec<double>(config["clip_actions_upper"])).view({1, -1});
        params_.clip_actions_lower = torch::tensor(
            ReadVectorFromYamlRec<double>(config["clip_actions_lower"])).view({1, -1});
    }

    params_.action_scale = config["action_scale"].as<double>();
    params_.hip_scale_reduction = config["hip_scale_reduction"].as<double>();
    params_.hip_scale_reduction_indices = ReadVectorFromYamlRec<int>(config["hip_scale_reduction_indices"]);
    ...
    params_.rl_kp = torch::tensor(ReadVectorFromYamlRec<double>(config["rl_kp"])).view({1, -1});
    params_.rl_kd = torch::tensor(ReadVectorFromYamlRec<double>(config["rl_kd"])).view({1, -1});
    ...
    if (config["abs"] && config["abs"]["contact_threshold"])
        params_.contact_threshold = config["abs"]["contact_threshold"].as<double>();
}

torch::Tensor StateRLRec::quatRotateInverse(const torch::Tensor& q, const torch::Tensor& v, const std::string& framework)
{
...
torch::Tensor StateRLRec::forward()
{
    ...
    if (!action_verified) {
        RCLCPP_INFO(rclcpp::get_logger("StateRLRec"), "[REC-ACTION] output shape: %ldx%ld (expected 1x%d)",
...
void StateRLRec::runModel()
{
    ...
    if (rl_step_count_ == 0) {
        double vx = ctrl_component_.recovery_twist_vx;
        double vy = ctrl_component_.recovery_twist_vy;
        double wz = ctrl_component_.recovery_twist_wz;
        // Fallback: if twist was never optimized (manual key 4 switch), use joystick
        if (vx == 0.0 && vy == 0.0 && wz == 0.0) {
            vx = std::clamp(control_.x, 0.0, 1.0) * 1.5;
            vy = std::clamp(control_.y, -1.0, 1.0) * 0.3;
            wz = std::clamp(control_.yaw, -1.0, 1.0) * 3.0;
        }
        obs_.commands = torch::tensor({{vx, vy, wz}});
        RCLCPP_INFO(rclcpp::get_logger("StateRLRec"),
            "[REC-TWIST] commands: vx=%.3f vy=%.3f wz=%.3f", vx, vy, wz);
    }
    obs_.base_quat = torch::tensor(robot_state_.imu.quaternion).unsqueeze(0);
...
void StateRLRec::setCommand() const
{
    for (int i = 0; i < 12; i++)
    {
        ...
        ctrl_interfaces_.joint_torque_command_interface_[i].get().set_value(
