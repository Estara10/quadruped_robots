...
add_paragraph_cn(
    '截至本报告撰写时，已完成仿真环境中的完整 ABS 管线搭建与验证，'
    '包括敏捷策略推理、RA 值网络评估、基于梯度下降的恢复策略 twist 优化、'
    '世界坐标系下的目标点导航与到达后自动重采样等核心功能。'
    '机器人能够在障碍物场景中自主导航至目标点并在遇到障碍时自动触发避障行为。'
)
...
completed = [
    ...
    ['Goal 重采样', '训练 episode reset 随机采样', '到达后 1.6s 站定, 随机重采样 [1.5,7.5]×[-2,2]m', '一致'],
...
add_paragraph_cn(
    ...
    '站定约 1.6 秒后自动随机采样新目标点。整个过程中 RA 值稳定在 -0.7 附近，'
    '未出现误触发恢复的情况。'
    '验证了基于 odometer 的世界坐标系目标导航和到达检测功能正确。'
)
...
add_paragraph_cn(
    '机器人在到达目标点并站定后，自动在当前位置的前方 1.5 到 7.5 米、侧向 -2.0 到 2.0 米范围内'
    '随机采样下一个目标点。该采样范围与原论文训练时的目标分布一致。'
